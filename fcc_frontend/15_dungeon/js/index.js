...
function buildRoom(gen, tl, tr, bl, br) {
  ...
  if (minX < t.marginTiles) {
    minX = t.marginTiles;
  }
  if (maxX > t.mapWidthTiles - t.marginTiles) {
    maxX = t.mapWidthTiles - t.marginTiles;
  }
  if (minY < t.marginTiles) {
    minY = t.marginTiles;
  }
  if (maxY > t.mapHeightTiles - t.marginTiles) {
    maxY = t.mapHeightTiles - t.marginTiles;
  }
...
function testClear(rooms, room) {
  ...
  for (var n = 0; n < numRooms; n++) {
    ...
    var clearThis = false;
    if (room.tl[0] >= rooms[n].tr[0] || room.tr[0] <= rooms[n].tl[0]) {
      clearThis = true;
    }
    if (room.tl[1] >= rooms[n].bl[1] || room.bl[1] <= rooms[n].tl[1]) {
      clearThis = true;
    }
    if (clearThis === false) {
      clear = false;break;
    ...
    for (var n = 0; n < numRoomsExisting; n++) {
      if (roomsExisting[n].gen == gen) {
        for (var side = 1; side <= 4; side++) {
          ...
          var br = [];
          if (side == 1) {
            bl = roomsExisting[n].tl;
          }
          if (side == 2) {
            tl = roomsExisting[n].tr;
          }
          if (side == 3) {
            tr = roomsExisting[n].br;
          }
          if (side == 4) {
            br = roomsExisting[n].bl;
          }
...
function createPlayerLayer(mapFeatures) {
  ...
  var inRoom = true;
  if (roomStart.tl[0] >= portal.tr[0] || roomStart.tr[0] <= portal.tl[0]) {
    inRoom = false;
  }
  if (roomStart.tl[1] >= portal.bl[1] || roomStart.bl[1] <= portal.tl[1]) {
    inRoom = false;
  }
  if (inRoom === true) {
...
function checkPixels(delX, delY, playerOnMap) {
  ...
  for (var p = 0; p < numPixelVals; p += 4) {
    if (pixelsMap[p] === c.wallColor[0] && pixelsMap[p + 1] === c.wallColor[1] && pixelsMap[p + 2] === c.wallColor[2]) {
      wall = true;
    }
    if (pixelsMap[p] === c.portalColor[0] && pixelsMap[p + 1] === c.portalColor[1] && pixelsMap[p + 2] === c.portalColor[2]) {
      portal = true;
    }
    if (pixelsMap[p] === c.weaponColor[0] && pixelsMap[p + 1] === c.weaponColor[1] && pixelsMap[p + 2] === c.weaponColor[2]) {
      weapon = true;
    }
    if (pixelsMap[p] === c.healthColor[0] && pixelsMap[p + 1] === c.healthColor[1] && pixelsMap[p + 2] === c.healthColor[2]) {
      health = true;
    }
    if (pixelsMap[p] >= c.enemyColor[0] - 100 && pixelsMap[p + 1] === c.enemyColor[1] && pixelsMap[p + 2] === c.enemyColor[2]) {
      enemy = [true, pixelsMap[p]];
    }
    if (pixelsMap[p] >= c.bossColor[0] - 100 && pixelsMap[p + 1] === c.bossColor[1] && pixelsMap[p + 2] === c.bossColor[2]) {
      boss = [true, pixelsMap[p]];
    }
  }

  var info = { wall: wall, portal: portal, weapon: weapon, health: health, enemy: enemy, boss: boss };
  return info;
}
  ...
  if (itemSet.length > 1) {
    ...
    for (var n = 0; n < numItems; n++) {
      ...
      if (distance < minDistance) {
        minDistance = distance;closest = n;
      }
    }
...
function attackEnemy(playerStrength, enemyStrength) {
  ...
  if (del < minDel) {
    damage = minDmg;injury = maxDmg;
  }
  if (del > maxDel) {
    damage = maxDmg;injury = minDmg;
  }
...
var Game = React.createClass({
  ...
  tryMove: function tryMove(delX, delY) {
  ...
  moveByKey: function moveByKey(e) {
  ...
  findWeapon: function findWeapon(delX, delY) {
  ...
  findHealth: function findHealth(delX, delY) {
  ...
  findBadGuy: function findBadGuy(delX, delY, info) {
  ...
  notWall: function notWall(delX, delY) {
    var playerUpdatedOnMap = movePlayer(delX, delY, this.state.playerOnMap);
    createView(playerUpdatedOnMap);
    this.setState({ playerOnMap: playerUpdatedOnMap });
  },
  ...
  render: function render() {
    ...
    return React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          "button",
          { className: "dirBtn", onClick: function onClick() {
              return _this.tryMove(-1, 0);
            } },
          "Left (j)"
        ),
        React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          "span",
          { className: "infoSpan" },
          React.createElement("span", { className: "infoDesc" })
        )
      ),
      React.createElement(
        "div",
      ...
      React.createElement(
        ...
        React.createElement(
          "button",
          { className: "otherBtn", onClick: this.newGame },
          "New Game"
        ),
        React.createElement(
          "button",
          { className: "otherBtn", onClick: toggleDarkness },
          "Darkness"
        )
      )
    );
...
ReactDOM.render(React.createElement(Game, null), document.getElementById('controlPanel'));